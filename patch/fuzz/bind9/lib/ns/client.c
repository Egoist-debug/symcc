...
ns__client_setup(ns_client_t *client, ns_clientmgr_t *mgr, bool new) {
	...
	if (new) {
		...
		*client = (ns_client_t){
			...
			.query = client->query,
			/*
			 * resolver-afl-symcc orchestrator 依赖 sendcb 拦截
			 * 客户端响应（进程内注入路径）。BIND 9.20 的
			 * ns__client_setup 重置会清零该字段，导致响应走真实
			 * 网络发送（connected UDP socket 发往自身监听端口），
			 * orchestrator 永远收不到。此处保留 sendcb，由
			 * orchestrator 在每次注入时重设。
			 */
			.sendcb = client->sendcb,
		};
