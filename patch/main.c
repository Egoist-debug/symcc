...
static int maxudp = 0;
static bool fuzz_resolver_afl_symcc_mode = false;
static const char *fuzz_resolver_afl_symcc_config = NULL;
...
static const char *
parse_optional_fuzz_mode_payload(const char *mode, const char *arg) {
	...
	if (arg[mode_len] != ':') {
		named_main_earlyfatal("unknown fuzzing type '%s'", arg);
	}

	if (arg[mode_len + 1] == '\0') {
		named_main_earlyfatal("empty fuzzing mode payload in '%s'", arg);
	}

	return arg + mode_len + 1;
}
...
static void
parse_fuzz_arg(void) {
	static const char resolver_afl_symcc_mode[] = "resolver-afl-symcc";

	fuzz_resolver_afl_symcc_mode = false;
	fuzz_resolver_afl_symcc_config = NULL;

	if (!strncmp(isc_commandline_argument, resolver_afl_symcc_mode,
		     sizeof(resolver_afl_symcc_mode) - 1))
	{
		fuzz_resolver_afl_symcc_mode = true;
		fuzz_resolver_afl_symcc_config = parse_optional_fuzz_mode_payload(
			resolver_afl_symcc_mode, isc_commandline_argument);
		named_g_fuzz_addr = fuzz_resolver_afl_symcc_config;
		named_g_fuzz_type = isc_fuzz_none;
	} else if (!strncmp(isc_commandline_argument, "client:", 7)) {
...
#ifdef ENABLE_AFL
	if (fuzz_resolver_afl_symcc_mode) {
		result = named_resolver_afl_symcc_orchestrator_start(
			fuzz_resolver_afl_symcc_config);
		if (result != ISC_R_SUCCESS) {
			named_main_earlyfatal(
				"resolver-afl-symcc orchestrator start failed: %s",
				isc_result_totext(result));
		}
	}
#endif /* ifdef ENABLE_AFL */
...
#ifdef ENABLE_AFL
	if (fuzz_resolver_afl_symcc_mode) {
		named_resolver_afl_symcc_orchestrator_stop();
	}
#endif /* ifdef ENABLE_AFL */
