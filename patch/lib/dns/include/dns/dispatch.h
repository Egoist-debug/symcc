...
/*%<
 * Optional hook for synthesizing a UDP response in-process.
 *
 * Returning ISC_R_SUCCESS means a synthetic response was produced in
 * 'response'. Returning ISC_R_TIMEDOUT simulates a timeout. Returning
 * ISC_R_NOTFOUND means the hook declined to handle the request and the
 * normal socket path should be used.
 */
...
/*%<
 * Installs or clears an optional in-process UDP response hook.
 */
