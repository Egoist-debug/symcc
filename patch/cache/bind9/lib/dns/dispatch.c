...
#ifndef TIME_NOW
#define TIME_NOW(tp) (*(tp) = isc_time_now())
#endif

#define isc_nm_tid isc_tid
#define isc_net_getudpportrange isc_net_getportrange
#define isc_nm_udpconnect_compat(mgr, local, peer, cb, cbarg, timeout, flags) \
	isc_nm_udpconnect(mgr, local, peer, cb, cbarg, timeout)
#define isc_nm_tcpdnsconnect_compat(mgr, local, peer, cb, cbarg, timeout, flags) \
	isc_nm_tcpconnect(mgr, local, peer, cb, cbarg, timeout)
...
struct dns_dispatchmgr {
	...
	isc_stats_t *stats;
	isc_nm_t *nm;
	dns_dispatch_udpresphook_t udpresphook;
	void *udpresphookarg;
...
udp_recv(isc_nmhandle_t *handle, isc_result_t eresult, isc_region_t *region,
	 void *arg);
static void
udp_dispatch_response(dns_dispentry_t *resp, isc_result_t eresult,
		      isc_region_t *region, const isc_sockaddr_t *peer,
		      const void *source);
static void
...
udp_dispatch_response(dns_dispentry_t *resp, isc_result_t eresult,
	...
	dres = dns_message_peekheader(&buffer, &id, &flags);
...
next:
	...
	if (response != NULL) {
		dispentry_log(resp, LVL(90), "UDP read callback on %p: %s",
			      source, isc_result_totext(eresult));
...
udp_recv(isc_nmhandle_t *handle, isc_result_t eresult, isc_region_t *region,
	...
	udp_dispatch_response((dns_dispentry_t *)arg, eresult, region, &peer,
			      handle);
}
...
void
dns_dispatchmgr_setudpresphook(dns_dispatchmgr_t *mgr,
			       dns_dispatch_udpresphook_t hook, void *hookarg) {
	REQUIRE(VALID_DISPATCHMGR(mgr));

	mgr->udpresphook = hook;
	mgr->udpresphookarg = hookarg;
}
...
udp_dispatch_connect(dns_dispatch_t *disp, dns_dispentry_t *resp) {
	...
	isc_nm_udpconnect_compat(disp->mgr->nm, &resp->local, &resp->peer,
...
tcp_dispatch_connect(dns_dispatch_t *disp, dns_dispentry_t *resp) {
	...
	case DNS_DISPATCHSTATE_NONE:
		...
		isc_nm_tcpdnsconnect_compat(disp->mgr->nm, &disp->local,
...
dns_dispatch_send(dns_dispentry_t *resp, isc_region_t *r) {
	...
	unsigned char response_buf[65535];
	isc_region_t response = { 0 };
	isc_result_t hookresult = ISC_R_NOTFOUND;
		...
		if (disp->mgr->udpresphook != NULL) {
			...
			if (hookresult != ISC_R_NOTFOUND) {
				dispentry_log(resp, LVL(90),
					      ...
					      isc_result_totext(hookresult));
				resp->sent(ISC_R_SUCCESS, NULL, resp->arg);
				udp_dispatch_response(resp, hookresult,
						      hookresult == ISC_R_SUCCESS
							      ? &response
							      : NULL,
						      hookresult == ISC_R_SUCCESS
							      ? &resp->peer
							      : NULL,
						      disp->mgr->udpresphook);
				return;
			}
		}
