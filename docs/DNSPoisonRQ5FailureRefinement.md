...
> 在当前多样本批次中，`unbound` 和 `smartdns` 的代表性样本均完成 replay，且 `bind9` 与 secondary 的 `parse_ok` 都为真。当前 `runtime_or_parse_failure` 更适合被保守地解释为 `oracle_parse_incomplete`，而不是目标未运行或目标崩溃。这一结果说明，我们当前的失败标签已经开始区分“语义证据不足”与“真实运行失败”。
