...
2. **Unbound shell 是 thin wrapper**：`unbound_experiment/run_unbound_afl_symcc.sh` 仅做路由、环境注入、路径检查与 timeout 外壳。
...
2. report 负责样本聚合与 triage 结果汇总。
3. campaign-report 负责活动级指标汇总。
...
2. `follow-diff` / `follow-diff-once` 从 source queue 消费样本。
3. 每个样本执行 paired replay（BIND9 + Unbound）并落单样本证据目录。
4. `triage-report` 先重写 triage 再汇总报告。
5. `campaign-report` 生成活动级统计产物。

> 说明：shell 仅是薄包装。dns-diff 的主流程入口固定为 `python3 -m tools.dns_diff.cli`。
...
1. follow_diff 能持续消费 source queue 并稳定生成 `sample_id` 目录。
2. 失败样本目录仍保留 `sample.meta.json.failure` + `*.stderr` + `triage.json`。
3. triage/report/campaign 三段命令可形成完整报告闭环。

### 产物验收

1. follow_diff 根目录可稳定生成 report 四件套。
2. campaign 报告目录可稳定生成 campaign 四件套。
3. 所有路径与默认值解释与当前仓库实现一致（以 Python CLI helper 逻辑为准）。
