...
4. `campaign-report` 输出目录中的 `evidence_bundle.json` 是 `publication_evidence_bundle` 合约落点，用来把 `summary.json`、`oracle_audit.tsv`、`oracle_reliability.json`、`failure_taxonomy.tsv`、`exclusion_summary.tsv` 与 `case_studies/index.tsv` 绑定到固定路径、字段路径和再生成命令。
5. 严格 1 小时正式闭环命令固定为 `python3 -m tools.dns_diff.cli campaign-close --budget-sec 3600`，或等价 wrapper 转发命令 `./unbound_experiment/run_unbound_afl_symcc.sh campaign-close --budget-sec 3600`。
6. 正式 deadline owner 固定在 Python `campaign-close`，summary 固定写入 `WORK_DIR/campaign_close.summary.json`，不是 wrapper 的 `DNS_DIFF_CLI_TIMEOUT_SEC`，也不是外层 shell `timeout`。
7. 只有 `follow-diff-window -> triage-report -> campaign-report` 三阶段全部成功，且 `campaign_close.summary.json` 显示 `status=success` 时，才算闭环成功。
8. `follow-diff` / `follow-diff-once` 仍保留为手工观察、局部检查、队列巡检入口，不作为 task 5 或正式实验结论命令。
...
- `sample.meta.json` 内嵌字段 `failure`（JSON 路径：`sample.meta.json.failure`）
- 对应阶段 `*.stderr`
- `triage.json`

要求：失败不吞样本，样本目录必须保留可定位的失败上下文。

### publication evidence bundle 合约

`campaign-report` 每次落盘时都必须生成 `campaign_reports/<timestamp>/evidence_bundle.json`，作为 `publication_evidence_bundle` 合约文件。

要求：

- `included / excluded / unknown` 仅表示 publication-facing 状态，不能直接改写成人工真值结论。
- `response_accepted`、`second_query_hit`、`cache_entry_created`、`oracle_diff` 等 proxy signal 只能作为筛选或归因线索，不能直接写成“漏洞已证明”。
- `case_studies/index.tsv` 只有在对应文件实际存在时才可当作已产出的 case-study 证据引用；若尚未执行 `case-study-export`，bundle 只能保留路径与再生成命令，不能把悬空路径写成已存在证据。
- `sample.meta.json.failure` 继续表示 `sample.meta.json` 内嵌 `failure` 对象的 JSON 路径，不是独立文件。
...
### 活动闭环产物（campaign 核心四件套 + publication evidence bundle）
...
- `repro_rate.tsv`
- `oracle_audit.tsv`
- `oracle_reliability.json`
- `failure_taxonomy.tsv`
- `exclusion_summary.tsv`
...
4. 失败样本目录仍保留 `sample.meta.json`（内嵌 `failure`）+ `*.stderr` + `triage.json`。
5. 闭环成功必须体现为 `follow-diff-window -> triage-report -> campaign-report` 三阶段全部成功，且 `campaign_close.summary.json` 显示 success。

### 产物验收

1. follow_diff 根目录可稳定生成 report 四件套。
2. campaign 报告目录可稳定生成 campaign 四件套。
3. campaign 报告目录可稳定生成 `evidence_bundle.json`，并显式映射 claim、字段路径与再生成命令。
4. `WORK_DIR/campaign_close.summary.json` 可稳定生成，且能作为正式结论 summary。
5. 所有路径与默认值解释与当前仓库实现一致（以 Python CLI helper 逻辑为准）。
...
5. 不在本文件添加长期研究路线扩张表述。
6. 不把 proxy signal 写成真值证明，也不把 `included / excluded / unknown` 混写成人工确认结论。
