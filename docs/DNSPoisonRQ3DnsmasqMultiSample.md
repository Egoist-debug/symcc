...
本文档记录当前 `dnsmasq` 上已经完成的 RQ3 多样本消融批次。它回答的是一个局部问题：在 `queue_limit=2`、`repeat=5`、`budget_sec=12` 的真实批次里，四个 Hybrid 变体是否已经拉开结果差异。
...
> 在 `dnsmasq` 上，`full_stack / afl_only / no_mutator / no_cache_delta` 四个变体均完成了 `repeat=5` 的稳定重复，并且在当前 `queue_limit=2` 的样本池上都收敛到相同的 `oracle_diff` 语义结果。也就是说，在这轮真实批次里，关闭 `SYMCC`、关闭 `DST1 mutator` 或关闭 `cache-delta` 尚未改变 `dnsmasq` 的主导语义分布，当前样本池还看不出明显的 hybrid 增益。
...
- 现有样本池下，Hybrid 组件的边际收益还没被稳定观测到
- 后续需要继续扩大 `queue_limit` 或扩充样本池，再判断增益
...
- 在当前 `dnsmasq + queue_limit=2 + repeat=5` 批次里，四个变体语义结果一致
- RQ3 还需要更大的样本池来验证是否存在稳定增益
