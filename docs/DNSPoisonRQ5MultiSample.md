...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `unbound` | `bind9_vs_unbound` | `5` | `ok` | `2.0` | `2.0` | `2.0` | `0.0` | `0.0` | `{"runtime_or_parse_failure": 2.0}` |
| `dnsmasq` | `bind9_vs_dnsmasq` | `5` | `ok` | `2.0` | `0.0` | `2.0` | `2.0` | `2.0` | `{"oracle_diff": 2.0}` |
| `smartdns` | `bind9_vs_smartdns` | `5` | `ok` | `2.0` | `2.0` | `2.0` | `0.0` | `0.0` | `{"runtime_or_parse_failure": 2.0}` |
| `maradns` | `bind9_vs_maradns` | `5` | `ok` | `2.0` | `0.0` | `2.0` | `2.0` | `2.0` | `{"oracle_diff": 2.0}` |
| `knot-resolver` | `bind9_vs_knot-resolver` | `5` | `ok` | `2.0` | `0.0` | `0.0` | `0.0` | `0.0` | `{"no_diff": 2.0}` |

## 当前可直接写入论文的表述

> 在 `full_stack` 主线、`queue_limit=2`、`repeat=5`、`budget_sec=12` 的真实批次中，5 个 secondary resolver 全部完成了稳定重复，`variance_status` 均为 `ok`。其中，`bind9_vs_dnsmasq` 与 `bind9_vs_maradns` 在所有重复中都稳定落为 `oracle_diff`，而 `bind9_vs_knot-resolver` 在同一批样本上持续表现为 `no_diff`。相对地，`bind9_vs_unbound` 与 `bind9_vs_smartdns` 当前仍主要表现为 `runtime_or_parse_failure`，说明当前稳定 transcript 子集对不同 resolver 的语义触发能力已经开始分化。
...
2. 每次运行都消费了 `2` 个稳定 transcript，而不是单个 `matrix-seed`
...
- `queue_limit=2` 只能说明“小规模多样本下结果稳定”，还不能替代 `1h/6h/24h` 主实验
...
1. 保持 `full_stack only`，把 `queue_limit` 从 `2` 提到 `4` 或 `8`
2. 保持 `queue_limit=2`，追加 `afl_only / no_mutator / no_cache_delta` 变体，补 RQ3 对照表
