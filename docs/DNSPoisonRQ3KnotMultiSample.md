...
本文档记录当前 `knot-resolver` 上已经完成的 RQ3 多样本消融批次。它回答的是第三个局部问题：在 `queue_limit=2`、`repeat=5`、`budget_sec=12` 的真实批次里，`knot-resolver` 上的四个 Hybrid 变体是否已经拉开结果差异。
...
> 在 `knot-resolver` 上，`full_stack / afl_only / no_mutator / no_cache_delta` 四个变体都完成了 `repeat=5` 的稳定重复，并且在当前 `queue_limit=2` 的样本池上全部收敛到 `no_diff`。这说明 `knot-resolver` 当前不仅是 RQ5 的稳定负对照，在 RQ3 的变体消融里也保持了稳定负对照属性。
...
- 当前小样本稳定 transcript 批次里，还看不出 `SYMCC / mutator / cache-delta` 改变主导语义分布
...
- 在当前 `knot-resolver + queue_limit=2 + repeat=5` 批次里，四个变体语义结果一致
- RQ3 的增益验证还需要补“无 high-value gate”对照，并扩大样本池或预算
