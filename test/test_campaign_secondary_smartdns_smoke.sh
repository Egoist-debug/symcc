...
WORKDIR="$(mktemp -d "${TMPDIR:-/tmp}/symcc-campaign-secondary-smartdns.XXXXXX")"
...
if comparability.get("aggregation_key", {}).get("resolver_pair") != "bind9_vs_smartdns":
...
if comparability.get("baseline_compare_key", {}).get("resolver_pair") != "bind9_vs_smartdns":
...
printf 'PASS: campaign secondary smartdns smoke test passed\n'
