...
MISSING_TRIAGE_ROOT="$WORKDIR/missing-triage-follow"
...
python3 - "$MISSING_TRIAGE_ROOT" <<'PY'
import json
import pathlib
import sys
...
MISSING_TRIAGE_STDERR="$WORKDIR/missing-triage.stderr"
if run_cli report --root "$MISSING_TRIAGE_ROOT" >/dev/null 2>"$MISSING_TRIAGE_STDERR"; then
	printf 'ASSERT FAIL: report 对缺失 triage.json 输入不应成功\n' >&2
	exit 1
fi
assert_file_contains "$MISSING_TRIAGE_STDERR" 'dns-diff: report 失败: 样本 semantic truth-source 无效:'
assert_file_contains "$MISSING_TRIAGE_STDERR" 'file=triage.json'
assert_file_contains "$MISSING_TRIAGE_STDERR" 'reason=文件缺失'
