...
mkdir -p "$WORKDIR/bind9-source" "$WORKDIR/dnsmasq-source" "$SAMPLE_DIR" "$RESPONSE_CORPUS_DIR"
...
python3 - "$RUN_ROOT/evidence_bundle.json" "$RUN_ROOT/case_studies/index.tsv" "$DNSLABCTL_BIN" <<'PY'
import csv
import json
import pathlib
import sys
...
if not replay_command.startswith(f"{dnslabctl_bin} sync-replay "):
    raise SystemExit(
