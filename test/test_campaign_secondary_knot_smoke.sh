...
WORKDIR="$(mktemp -d "${TMPDIR:-/tmp}/symcc-campaign-secondary-knot.XXXXXX")"
...
if comparability.get("aggregation_key", {}).get("resolver_pair") != "bind9_vs_knot-resolver":
...
if comparability.get("baseline_compare_key", {}).get("resolver_pair") != "bind9_vs_knot-resolver":
...
if parse_ok.get("knot-resolver") is not True:
    raise SystemExit(f"ASSERT FAIL: knot-resolver parse_ok 未映射到 audit 列: {parse_ok!r}")
artifacts = bundle["raw_sample_root"]["claim_review_artifacts"]
if "knot-resolver.stderr" not in artifacts:
    raise SystemExit(f"ASSERT FAIL: evidence bundle 缺少 knot-resolver.stderr: {artifacts!r}")
PY

printf 'PASS: campaign secondary knot smoke test passed\n'
