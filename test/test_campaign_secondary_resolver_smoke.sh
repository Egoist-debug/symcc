...
WORKDIR="$(mktemp -d "${TMPDIR:-/tmp}/symcc-campaign-secondary.XXXXXX")"
...
if comparability.get("aggregation_key", {}).get("resolver_pair") != "bind9_vs_dnsmasq":
...
if comparability.get("baseline_compare_key", {}).get("resolver_pair") != "bind9_vs_dnsmasq":
...
printf 'PASS: campaign secondary resolver smoke test passed\n'
